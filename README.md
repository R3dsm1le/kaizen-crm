...
Set `DATABASE_URL` (+ optional vars from `.env.example`) in the Vercel project — on Vercel the env var is **required** (the in-app first-run setup can't persist on serverless; it's for local and desktop use). Use Supabase's **pooler** connection string (Connect dialog → Session or Transaction pooler, host ends in `pooler.supabase.com`); the direct `db.…supabase.co` URL is IPv6-only and unreachable from Vercel. Set `CRON_SECRET` to protect the automation endpoint.

Easiest of all: install the **Vercel × Supabase integration** — its injected `POSTGRES_URL` (and `NEXT_PUBLIC_SUPABASE_*` auth vars) are picked up automatically, and tables are created on first load. Since the integration exposes the Supabase anon key to browsers, run `db/rls.sql` in the Supabase SQL editor afterwards to lock the tables down against direct Data API access.
